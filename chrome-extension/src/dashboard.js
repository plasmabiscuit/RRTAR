...
const TAB_DEFS = {
  keyperson: {
    ...
    steps: [
      ...
      ["extract", "Parse XFA XML and attachments from input PDFs", false],
      ["validate", "Schema and business-rule validation", false],
      ["normalize", "Build import_manifest.json for review", false],
  ...
  budget: {
    ...
    steps: [
      ["extract-budget", "Parse budget PDFs into reviewable data", false],
      ["normalize-budget", "Build budget_manifest.json for review", false],
  ...
  "performance-site": {
    ...
    steps: [
      ["extract-performance-site", "Parse Performance Site PDFs into reviewable data", false],
      ["normalize-performance-site", "Build performance_site_manifest.json for review", false],
  ...
  const content = `
    <div class="row2">
      <div class="card">
        ...
        <div class="chrome-box">
          ...
          <span class="chrome-info">${escapeHtml(renderPipelineSummary(preview))}</span>
        </div>
      </div>
      <div class="card">
        ...
        <label id="drop-zone" class="drop-zone">
          ...
          <p style="font-size:.78rem;margin-top:.2rem">Stored clientside in the extension for this tab.</p>
        </label>
    ...
    <div class="card" style="margin-bottom:1.2rem">
      ...
      <form id="manifest-import-form" class="manifest-save-bar">
        ...
        <span class="manifest-save-note">This shell mirrors ui.py, but non-automation pipeline stages are still being ported. Import current manifest JSON to work clientside.</span>
      </form>
...
function renderPipelineSummary(preview) {
  const lines = Array.isArray(preview?.lines) ? preview.lines : [];
  return lines.join(" · ") || "Open Grants.gov, import a manifest, then run Automate.";
}

function renderFileTable(files) {
  const sourceFiles = files.filter((file) => file.role === "source-pdf");
  if (!sourceFiles.length) {
    return '<p class="empty-note">No PDFs yet — drop some above</p>';
  }
  const rows = sourceFiles.map((file) => {
    const remove = `<button type="button" class="btn-danger btn-sm" data-remove-file="${escapeAttr(file.id)}">${icon("close", "icon-btn")}</button>`;
    return `<tr><td><strong style="font-size:.83rem">${escapeHtml(file.name)}</strong></td><td class="sub-dim">${formatSize(file.size)}</td><td><span class="badge ok">stored</span> <span class="sub-info">clientside</span></td><td style="text-align:center">${remove}</td></tr>`;
  }).join("");
  return `<table><thead><tr><th>File</th><th>Size</th><th>Status</th><th></th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
function renderKeypersonManifest(manifest, preview) {
  ...
  return `<div class="kp-manifest"><table><thead><tr><th>Incl.</th><th>Name</th><th>Contact</th><th>Organization</th><th>Role / Credential</th><th>Attachments</th><th>Warnings</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function renderBudgetManifest(manifest) {
  return manifest.map((entry, index) => {
    ...
    const periods = Array.isArray(entry.periods) ? entry.periods.map((period) => {
      return `<div class="budget-period-hdr">Period ${escapeHtml(String(period.period_index || "?"))}: ${escapeHtml(period.start_date || "")} – ${escapeHtml(period.end_date || "")}</div>
      <div class="sub-info">Direct Costs: ${escapeHtml(formatMoney(period.direct_costs))} · Total Costs: ${escapeHtml(formatMoney(period.total_costs))}</div>`;
    }).join("") : "";
    return `<div class="card budget-row-card">
      ...
      <div class="sub-info" style="font-size:.8rem;margin:.45rem 0">${icon("file", "icon-status")} Budget Justification: ${escapeHtml(fileLabel(just.path))}</div>
...
async function uploadFiles(fileList) {
  ...
  setStatus(`Stored ${files.length} PDF file${files.length === 1 ? "" : "s"} clientside for ${TAB_DEFS[activeTab].title}.`, "");
  await refreshState();
}
...
async function runAutomate() {
  ...
  setStatus(`Running ${TAB_DEFS[activeTab].automateStep} against the active Grants.gov tab...`, "");
