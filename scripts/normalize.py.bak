...
def sha1_b64(path: Path) -> str | None:
...
def _form_ns(el: etree._Element | None) -> str:
    return etree.QName(el.tag).namespace if el is not None else FORM_NS


def _resolve_attachment(desc_pattern: str, att_dir: Path,
                        last_name: str = "", label: str = "") -> tuple[str | None, str | None]:
...
def _classify_attachment_pdf(text: str) -> str | None:
    upper = text.upper()
...
def _parse_attachment_only_pdf(path: Path) -> dict | None:
    text = _pdf_text(path)
    if not text:
        return None
