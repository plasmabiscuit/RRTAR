...
def _t(el: etree._Element | None, tag: str, ns: str = FORM_NS) -> str:
    if el is None:
        return ""
...
def _parse_personnel_row(el: etree._Element | None) -> dict:
    if el is None:
        return {}
    return {
...
def _resolve_justification(stem: str) -> tuple[str | None, str | None]:
    if ATT_DIR.is_dir():
