...
Usage:
  python automate_budget.py [--manifest review/budget_manifest.json] [--dry-run] [--save]
"""
...
def _load_budget_metadata(stem: str) -> dict:
    if not stem:
        return {}
    meta_path = BASE / "input-budget-pdfs" / f"{stem}.budget.json"
...
def _resolve_fee(period: dict) -> str:
    fee = period.get("fee")
    if fee not in (None, ""):
        return str(fee)

    total_costs_fee = _parse_amount(period.get("total_costs_fee"))
    total_costs = _parse_amount(period.get("total_costs"))
    if total_costs_fee is None or total_costs is None:
        return ""

    derived_fee = total_costs_fee - total_costs
    if derived_fee <= 0:
        return ""
    return f"{derived_fee:.2f}"
...
def _fill_modal(frame, person: dict, dry_run: bool) -> dict:
    ...
    modal_fields = {
        "#prefix":          prefix if prefix not in {"Other", "OTHER"} else "",
        "#otherPrefix":     prefix if prefix in {"Other", "OTHER"} else "",
        "#firstName":       person.get("first_name", ""),
        "#middleName":      person.get("middle_name", ""),
        "#lastName":        person.get("last_name", ""),
        "#suffix":          suffix if suffix not in {"Other", "OTHER"} else "",
        "#otherSuffix":     suffix if suffix in {"Other", "OTHER"} else "",
...
def _add_key_persons(frame, period_idx: int, persons: list[dict], dry_run: bool) -> list[dict]:
    ...
    target_person_rows = len(persons) if period_idx == 1 else len(persons) + 1

    if period_idx > 1 and existing_rows < target_person_rows:
        for _ in range(existing_rows + 1, target_person_rows + 1):
    ...
    for person_idx, person in enumerate(persons, 1):
        ...
        row_idx = person_idx if period_idx == 1 else person_idx + 1
    ...
    if period_idx > 1 and existing_rows > target_person_rows:
        for row_idx in range(existing_rows, target_person_rows, -1):
...
def _fill_section_e(frame, period_idx: int, section_e: dict, dry_run: bool) -> dict:
    P = period_idx
    filled = {}
    skipped = {}

    row_fields = {
        f"fundsRequestedE{P}_1": section_e.get("tuition_fees_health_insurance", ""),
        f"fundsRequestedE{P}_2": section_e.get("stipends", ""),
        f"fundsRequestedE{P}_3": section_e.get("travel", ""),
        f"fundsRequestedE{P}_4": section_e.get("subsistence", ""),
        f"listE{P}_5": section_e.get("other_description", ""),
        f"fundsRequestedE{P}_5": section_e.get("other_cost", ""),
        f"numberOfParticipants{P}": (
            section_e.get("number_of_participants", "")
            or section_e.get("number_of_participants_trainees", "")
        ),
    }
    for fid, val in row_fields.items():
        err = _set_field(frame, f"#{fid}", val, dry_run)
...
def _fill_section_j(frame, period_idx: int, period: dict, dry_run: bool) -> dict:
    fid = f"totalFundsRequestedJ{period_idx}"
    value = _resolve_fee(period)
    err = _set_field(frame, f"#{fid}", value, dry_run)
    if err:
        return {"filled": {}, "skipped": {fid: err}}
    return {"filled": {fid: value}, "skipped": {}}
...
def _resolve_attachment_path(path_str: str | None) -> Path | None:
    if not path_str:
        return None
    path = Path(path_str)
...
def _set_attachment_input(frame, selector: str, path_str: str | None, dry_run: bool) -> str | None:
    path = _resolve_attachment_path(path_str)
    if not path:
        return None
    if not path.exists():
        return f"File not found: {path}"
    if dry_run:
        return f"DRY-RUN: would upload {path.name}"
    try:
        frame.locator(selector).set_input_files(str(path))
        return f"uploaded {path.name}"
    except Exception as exc:
        return f"ERROR: {exc}"


def _pick_first(mapping: dict, keys: list[str]) -> str | None:
    for key in keys:
...
def _period_attachment_spec(entry: dict, period: dict, metadata: dict) -> dict:
    period_idx = period["period_index"]
    merged: dict = {}

    for source in (
        (metadata.get("period_attachments") or {}).get(str(period_idx), {}),
        (entry.get("period_attachments") or {}).get(str(period_idx), {}),
        period.get("attachments") or {},
    ):
        if isinstance(source, dict):
            merged.update(source)

    return {
        "senior_key_persons": _pick_first(merged, [
            "senior_key_persons",
            "senior_key_person",
...
def _attach_period_files(frame, period: dict, entry: dict, metadata: dict, dry_run: bool) -> dict:
    period_idx = period["period_index"]
    spec = _period_attachment_spec(entry, period, metadata)
    return {
        "senior_key_persons": _set_attachment_input(
            frame,
            f"#seniorKeyPersonFile{period_idx}",
            spec.get("senior_key_persons"),
            dry_run,
        ),
        "equipment": _set_attachment_input(
            frame,
            f"#equipmentFile{period_idx}",
            spec.get("equipment"),
            dry_run,
        ),
    }
...
def _attach_justification(frame, path_str: str | None, dry_run: bool) -> str | None:
    return _set_attachment_input(frame, "#budgetJustificationFile", path_str, dry_run)
...
def run(manifest_path: Path | None = None, dry_run: bool = False, save: bool = False) -> None:
    ...
    with sync_playwright() as pw:
        ...
        for entry_i, entry in enumerate(entries):
            ...
            metadata = _load_budget_metadata(entry.get("source_pdf", ""))
            ...
            for period in entry_periods:
                ...
                print("  Section E: filling participant / trainee support …")
                e_result = _fill_section_e(frame, P, sec_e, dry_run)
                print(f"    filled={len(e_result['filled'])}  skipped={len(e_result['skipped'])}")
                period_log["section_e"] = e_result
                ...
                # Section J — Fee
                print("  Section J: filling fee …")
                j_result = _fill_section_j(frame, P, period, dry_run)
                print(f"    filled={len(j_result['filled'])}  skipped={len(j_result['skipped'])}")
                period_log["section_j"] = j_result

                attachment_result = _attach_period_files(frame, period, entry, metadata, dry_run)
                if any(attachment_result.values()):
                    print("  Period attachments:")
                    for key, status in attachment_result.items():
                        if status:
                            print(f"    {key}: {status}")
                period_log["period_attachments"] = attachment_result
        ...
        if save:
            ...
            save_status = "skipped (auto-save disabled)"
            ok_status = None
            print("\nSave skipped (auto-save disabled). Use --save to persist changes.")
            if not dry_run:
                page.screenshot(path=str(SS_DIR / "999_after_fill.png"))
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    run(args.manifest, args.dry_run, args.save)
